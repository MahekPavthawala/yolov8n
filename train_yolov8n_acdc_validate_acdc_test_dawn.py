...
Pipeline
--------
1) Start from COCO-pretrained yolov8n.pt
2) Train on ACDC train_gt
3) Validate on ACDC val_gt
4) Evaluate on DAWN test split
5) Save DAWN prediction images with boxes/classes/confidence
6) Save summaries and comparison CSVs in one main folder
...
def print_quick_report(model_path: Path, acdc_metrics, dawn_metrics, run_dir: Path):
    ...
    print(f"\nDAWN predictions  : {PROJECT_ROOT / f'{run_dir.name}_predict_dawn'}")
